...
public class EarthquakeCityMap extends PApplet {
	...
	public void mouseMoved()
	{
		...
		m.addAll(quakeMarkers);
		m.addAll(cityMarkers);
		selectMarkerIfHover(m);*/
	...
	private void hideMarkers(CommonMarker lastclicked ){
		...
		if(lastclicked  instanceof EarthquakeMarker){
			...
			double threatdis = ((EarthquakeMarker)lastClicked).threatCircle();
			for(Marker m : quakeMarkers){
				if(!lastClicked.equals(m))
					m.setHidden(true);
			}
			for(Marker m: cityMarkers){
				if(lastClicked.getDistanceTo(m.getLocation())>threatdis)
					m.setHidden(true);
			}
